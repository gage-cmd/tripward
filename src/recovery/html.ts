...
import { receiptHtmlPath, escapeHtml } from "../receipt/html.js";
...
function safetyStrip(preview: RecoveryPreview): { id: string; title: string; body: string; cls: string } {
  ...
  return { id: "strip-intact", title: "Preexisting work intact", body: INTACT_HINT, cls: "strip-ok" };
}
...
export function renderRecoveryHtml(
  ...
  options: { receiptPresent?: boolean } = {},
): string {
  ...
  const apply =
    ...
    ${counts.selectable === 0 ? `<p class="secondary">No selectable paths. Recovery will not change the worktree.</p>` : ""}
    <div class="cmd-row">
...
(function () {
  ...
  function compose(digest, runId, paths) {
    if (!paths.length) {
      return "tripward restore --confirm --digest " + digest + " --paths= " + runId;
    }
  ...
  function rewrite() {
    ...
    var next = compose(digest, runId, selectedPaths());
    field.value = next;
    var copy = field.parentNode && field.parentNode.querySelector("[data-copy]");
    if (copy) copy.setAttribute("data-copy", next);
  }
...
export function writeRecoveryHtml(runDirectory: string, preview: RecoveryPreview): string {
  ...
  writeFileSync(html, renderRecoveryHtml(preview, { receiptPresent }), { mode: 0o600 });
  return html;
}
