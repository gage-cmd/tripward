...
Consequences: A default `tripward run` will not deny a listed tool or trip exact-repeat. Operators who want Gate 0 behavior must flip enforce or use `--preset spike`. Time-fuse at the supervisor still honors `max_elapsed_seconds` (process envelope), even when hook-level `TIME_FUSE` is remapped to warn in shadow.
