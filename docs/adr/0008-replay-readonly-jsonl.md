...
Decision: `tripward replay` is a read-only parser over `runs/*/journal.jsonl` and `receipt.json`. It never creates or writes files. Events in the known type set are **observed**; unknown types and receipt-only reconstructions are **inferred**. Privacy limitations are printed every time.
