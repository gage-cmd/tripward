...
Decision: The spike has no daemon and no loopback HTTP. `fusecap run` supervises a child. `fusecap hook` is a Claude Code command hook that reads stdin, appends the journal, evaluates policy, and writes `control/stop.json` / `control/handshake.json` under the run directory. IPC is environment variables (`FUSECAP_RUN_DIR`) plus those files.
...
Consequences: No concurrent-run accounting, no remote hook HTTP. Fail-visibly if `FUSECAP_RUN_DIR` is unset (hooks cannot silently no-op).
