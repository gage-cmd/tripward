...
npx tsx /path/to/fusecap/src/cli.ts run -- claude
...
npx tsx /path/to/fusecap/src/cli.ts run --preset spike -- claude
```

Edit `.fusecap/policy.json` and set `"mode": "enforce"` if you already customized the file.
...
| Real run | A Claude Code session launched with `fusecap run` (not `--stub`) that produces a sealed receipt. Stub/CI runs do **not** count. | Gage / CoS | No |
| Legitimate signal | A `detector.signaled` or `policy.signaled` (shadow) **or** a `fuse.tripped` (enforce) that the operator agrees was warranted — not a synthetic stub scenario. | Gage / CoS + operator note | No |
...
| `fusecap run --mode` | Per-run override |
