...
`package.json` `bin` exposes `tripward`. A leftover `fusecap` bin may exist for existing installs only. User-facing HTML and buyer docs brand Tripward and clone `gage-cmd/tripward` only.
